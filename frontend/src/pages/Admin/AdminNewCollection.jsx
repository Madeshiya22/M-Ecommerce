...
export default function AdminNewCollection() {
  ...
  const { data: productsData, isLoading } = useQuery({
    queryKey: ['admin-new-collection', page, search],
    queryFn: () => productService.getAll({ page, limit: 15, keyword: search || undefined, newArrival: true }),
    select: (res) => res.data,
  });
  ...
  const createMut = useMutation({
    ...
    onSuccess: () => { toast.success('Product created!'); qc.invalidateQueries(['admin-new-collection']); closeModal(); },
    onError: (err) => toast.error(err.response?.data?.message || 'Error'),
  });

  const updateMut = useMutation({
    mutationFn: ({ id, data }) => productService.update(id, data),
    onSuccess: () => { toast.success('Product updated!'); qc.invalidateQueries(['admin-new-collection']); closeModal(); },
    onError: (err) => toast.error(err.response?.data?.message || 'Error'),
  });

  const deleteMut = useMutation({
    mutationFn: (id) => productService.delete(id),
    onSuccess: () => { toast.success('Product deleted'); qc.invalidateQueries(['admin-new-collection']); },
    onError: (err) => toast.error(err.response?.data?.message || 'Error'),
  });
  ...
  return (
    <div>
      <div className="admin-section-header">
        <h1 className="admin-section-title">New Collection</h1>
        <button className="btn btn-primary btn-sm" onClick={openCreate}><FiPlus size={14} /> Add Product to New Collection</button>
      </div>
